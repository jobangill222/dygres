...
function AppRouter() {
  ...
  const conditionHandler = async () => {
    ...
    if (userData && userData.role === "user") {
      ...
      if (
        userData &&
        userData.isEmailVerify === 0 &&
        userData.isEmailVerificationReminderSent === 1
      ) {
        if (pathName !== "/personalinformation") {
          toast(
            "To continue using dygres, please verify your email address by clicking the verify e-mail button."
          );
      ...
      if (
        userDataStats &&
        userDataStats.totalFollowers > 49000 &&
        userData.isPhotoVerify === 0
      ) {
        if (pathName !== "/personalinformation") {
          toast(
            "You have reached 50k followers, please submit your image for verification via the upload image button. Please make sure to follow the instructions."
          );
          navigate("/personalinformation");
          return;
        }
        ...
        {!userToken ? (
          <>
            <Route element={<AuthLayout />}>
              ...
              <Route exact path="/share/:username" element={<SignUp />} />
            ...
            {user?.role === "user" || user?.role === "dummy" ? (
              <>
                <Route element={<SiteLayout />}>
                  ...
                  <Route
                    exact
                    path="/SinglePostDetail/:postIdForSinglePost"
                    element={<SinglePostDetail />}
                  />
                ...
                <Route element={<ProfileLayout />}>
                  ...
                  <Route
                    exact
                    path="/Usersprofile/:userIDForProfile"
              ...
              <>
                <Route element={<AdminLayout />}>
                  ...
                  <Route
                    exact
                    path="admin/blocked-user"
                    element={<BlockedUserList />}
                  />
                  <Route
                    exact
                    path="admin/verification-request-list"
                    element={<VerificationRequestList />}
                  />

                  <Route
                    exact
                    path="admin/official-user-list"
                    element={<OfficialUserList />}
                  />
                  ...
                  <Route
                    exact
                    path="admin/post/single-post/:postIdForSinglePost"
                    element={<SinglePostDetailAdmin />}
                  />

                  <Route
                    exact
                    path="admin/hashtagPosts"
                    element={<HashtagPostsAdmin />}
                  />
                  ...
                  <Route
                    exact
                    path="admin/flagpost/:userID"
                    element={<FlagPost />}
                  />
                  <Route
                    exact
                    path="admin/userverification/:userIDForVerification"
                  ...
                  <Route
                    exact
                    path="admin/editpackage/:packageID"
                    element={<EditPackage />}
                  />
