...
const FlagPost = () => {
    ...
    const { postList, popupType, setPostList, getUsersFlaggedPostDContext, postIDForAwardOfPost, isLoading, setIsLoading, getOtherUserDetailByUserIDDContext } = useContext(DContext);
    ...
    const getUsersFlaggedPost = async () => {
        try {
            setIsLoading(true);
            let pageNumberOfPostList = 1;
            const axiosRes = await getUsersFlaggedPostDContext(userID, pageNumberOfPostList);
    ...
    const appendNextList = async () => {
        ...
        let axiosRes;
        axiosRes = await getUsersFlaggedPostDContext(userID, pageNumberOfPostList);
        if (axiosRes.status === "success") {
